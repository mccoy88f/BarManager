...
- **Logo nell'intestazione email**: se impostato un logo in Impostazioni locale (`Venue.logoUrl`, distinto dalla copertina del menù pubblico), viene anteposto come immagine in alto in tutte le email HTML di questo modulo (cliente e locale) — gestito centralmente da `MailService.send` (`common/mail/mail.service.ts`, §7), non da ciascun template, così un domani anche altri moduli (es. ordini fornitori) possono riusarlo passando lo stesso parametro.

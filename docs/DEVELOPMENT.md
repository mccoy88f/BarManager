...
- **Prenotazioni (§5.7)** — implementata in v1 con queste scelte di default (da confermare/rivedere con l'utente):
  ...
  - ~~v1 non combina più tavoli per un unico gruppo grande~~ — **risolto per l'assegnazione manuale**: se nessun tavolo singolo basta, la prenotazione resta `PENDING` senza tavolo assegnato (visibile nel tab "Senza tavolo" della coda) ma l'admin può ora scegliere ed assegnare più tavoli insieme dallo stesso campo (Autocomplete multiplo), sia in coda sia dall'aggiunta manuale — modellato con una relazione molti-a-molti (`ReservationTable`). Resta **non** automatica: l'assegnazione automatica del widget pubblico (best-fit) sceglie sempre un solo tavolo, non ne combina mai più di uno.

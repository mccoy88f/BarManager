...
- **Consenso marketing esplicito (`marketingConsent`)**: raccolto con una casella dedicata ("Accetto di ricevere comunicazioni promozionali...", facoltativa e deselezionata di default) nel widget pubblico di prenotazione (`Reservation.marketingConsent`), propagato al `Customer` corrispondente a ogni prenotazione (l'ultimo valore dichiarato vince, quindi un cliente può anche revocarlo prenotando di nuovo con la casella deselezionata). Modificabile anche a mano dall'admin (CRUD/import xlsx). A differenza di `lastReservationAt`, questo campo **è** un consenso esplicito ai fini GDPR/e-privacy, perché richiede un'azione volontaria del cliente (non è mai implicito nell'aver semplicemente prenotato).
- **CRUD manuale** (`/customers`, pagina "Clienti"): l'admin può aggiungere, modificare ed eliminare clienti indipendentemente dalle prenotazioni (es. per censire un cliente abituale prima ancora che prenoti online). Le date di prima/ultima prenotazione e il contatore sono di sola lettura quando derivano dal flusso automatico, ma sovrascrivibili tramite import xlsx (v. sotto).
...
- **Pagina pubblica "gestisci i tuoi dati personali"** (`/privacy?token=...`, nessun login, link in fondo a ogni email di prenotazione al cliente — §5.7): identificata da `Customer.privacyToken`, un token opaco **unico a livello globale** (non solo per locale, a differenza di `[venueId, email]`) perché è l'unico dato nel link. Generato alla prima prenotazione (`CustomersService.recordReservation`) e mai più cambiato (un link mandato in un'email precedente deve continuare a funzionare); per un cliente creato prima di questo campo, o toccato da un'azione che non passa da `recordReservation` (accetta/rifiuta/annulla/proponi orario), viene generato al bisogno da `CustomersService.ensurePrivacyToken`. Due sole azioni, sullo stesso principio di un link di cancellazione da una mailing list: **togliere il consenso marketing** (`PATCH /public/customers/privacy/opt-out`) o **eliminare la propria scheda cliente** (`DELETE /public/customers/privacy`) — quest'ultima elimina solo l'anagrafica (`Customer`: nome/cognome/email/telefono/note/consenso), non le prenotazioni già effettuate, che restano nello storico operativo del locale (§5.7); la pagina lo dichiara esplicitamente prima di confermare. Endpoint pubblici in `public-customers.controller.ts`, distinti da quelli admin (`customers.controller.ts`, che richiedono login).

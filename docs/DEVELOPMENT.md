...
- **Prenotazioni (§5.7)** — implementata in v1 con queste scelte di default (da confermare/rivedere con l'utente):
  ...
  - Il cliente **non può** annullare/modificare la propria prenotazione da solo: solo l'admin può farlo (accetta/rifiuta/annulla/cambia orario), non c'è un link di self-service nell'email — l'unica azione che il cliente può fare autonomamente è **confermare un nuovo orario proposto dal locale** (link dedicato nell'email, non un'annullamento/modifica libera).

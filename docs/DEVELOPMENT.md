...
- Backend: `Venue.openingHours` (Json, 7 giorni × fino a 2 fasce ciascuno) sostituisce i vecchi campi fissi `lunchStart/lunchEnd/dinnerStart/dinnerEnd`, con util condivisa (`opening-hours.ts`) usata sia dal menù (§5.4) sia dalle prenotazioni; validazione server-side dell'orario prenotabile per giorno/fascia e ai 15 minuti; flusso proponi/confermi cambio orario (`proposedReservedAt` + email con link di conferma cliente, stesso pattern anti-phishing di Accetta/Rifiuta); durata di occupazione sovrascrivibile per singola prenotazione (`Reservation.slotDurationMinutes`); tavoli occupati resi visibili (non bloccati) in coda, nel dialog di aggiunta manuale e nella pagina pubblica di gestione, tramite un nuovo endpoint `GET /reservations/table-availability`; endpoint di modifica generale (`PATCH /reservations/:id`) per correggere dati/note/durata senza richiedere conferma del cliente.
- Frontend: editor settimanale orari di apertura in Impostazioni locale; widget pubblico `/prenota` aggiornato (riepilogo orari per giorno, avviso se il giorno scelto è chiuso, step del campo orario a 15 minuti); admin Prenotazioni con pulsanti "Cambia orario"/"Modifica", indicazione tavoli occupati nei menu di scelta tavolo e nel dialog di aggiunta manuale (con durata opzionale); pagina pubblica di gestione con banner di conferma del nuovo orario proposto.
...
- Test Jest aggiornati/estesi (menù + prenotazioni, incluso un test che riproduce esattamente lo scenario del bug).

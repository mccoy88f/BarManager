...
### 5.8 Modulo Clienti *(implementata — v1: CRUD manuale + creazione/aggiornamento automatico da ogni prenotazione; l'invio di campagne resta fuori scope, vedi sotto)*
...
- **`lastReservationAt`**: aggiornato a ogni nuova prenotazione (al momento in cui viene effettuata la richiesta, non alla data/ora futura della prenotazione stessa) — pensato anche come riferimento per un futuro invio di campagne, cioè un proxy di "quanto è recente il rapporto col cliente", **non** un consenso esplicito al marketing.
- **`reservationsCount`**: contatore che si incrementa a ogni prenotazione della stessa email.
- **CRUD manuale** (`/customers`, pagina "Clienti"): l'admin può aggiungere, modificare ed eliminare clienti indipendentemente dalle prenotazioni (es. per censire un cliente abituale prima ancora che prenoti online). Le date di prima/ultima prenotazione e il contatore sono di sola lettura, calcolati solo dal flusso automatico.
...
- **Non implementato** (fuori scope di questa v1, vedi §10): l'invio vero e proprio di campagne pubblicitarie via email/SMS/WhatsApp. Nota importante per quando verrà implementato: `lastReservationAt` da solo **non** costituisce un consenso esplicito al marketing ai sensi GDPR/e-privacy — un vero invio di campagne richiederà un campo di consenso esplicito raccolto separatamente (es. una casella "accetto di ricevere comunicazioni promozionali" nel widget di prenotazione o in un'area cliente dedicata), non semplicemente dedotto dall'aver prenotato.
...
**Fase 1-quater — Modulo Clienti (completata, §5.8)**
...
- Non implementato (v1, resta in §10): invio di campagne pubblicitarie via email/SMS/WhatsApp — richiede provider terzi e un consenso di marketing esplicito, non deducibile dalle sole date di prenotazione già tracciate.
...
- **Clienti (§5.8)** — implementata in v1 come CRUD + tracciamento automatico date, ma resta da confermare con l'utente:
  - Se/quando implementare l'invio vero e proprio di campagne (email/SMS/WhatsApp): richiede provider terzi (es. un ESP per l'email, Twilio o simile per SMS/WhatsApp) e, soprattutto, un consenso esplicito al marketing raccolto separatamente — `lastReservationAt` da solo non basta ai fini GDPR/e-privacy.
  - Se aggiungere un campo di consenso esplicito (es. una casella nel widget di prenotazione) già in questa fase, in vista delle campagne, oppure rimandarlo a quando le campagne verranno effettivamente implementate.

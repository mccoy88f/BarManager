...
### 5.7 Prenotazioni tavoli *(implementata — v1: assegnazione a un solo tavolo, overbooking configurabile (illimitato o soglia di posti extra, non solo blocco rigido), nessun accorpamento tavoli, nessuna cancellazione self-service; vedi §10 per le estensioni ancora aperte)*
...
- **Prenotazioni**: coda con tab per stato (`PENDING`/`CONFIRMED`/`REJECTED`/`CANCELLED`) più un tab **"Senza tavolo"** che raggruppa, a prescindere dallo stato, tutte le prenotazioni attive ancora senza un tavolo assegnato (sia richieste online rimaste manuali, sia aggiunte a mano senza sceglierne uno). Azioni rapide **Accetta**/**Rifiuta** (il rifiuto richiede un motivo, riportato al cliente via email); **riassegnazione manuale** del tavolo in qualsiasi momento, anche su una prenotazione già confermata — il menu di scelta del tavolo **esclude del tutto** quelli occupati da un'altra prenotazione attiva nello stesso orario: non compaiono come opzione, e assegnarli è comunque rifiutato lato server (a differenza della capienza totale/overbooking, qui non c'è override di backoffice — due prenotazioni non possono mai condividere lo stesso tavolo nello stesso momento). Contatore "posti disponibili" per data/servizio sempre visibile.
...
- **Aggiunta manuale** (telefono, di persona, ecc.): lo staff può sempre registrare una prenotazione dal backoffice, con gli stessi campi del widget pubblico (più una durata di occupazione opzionale, se diversa dal default del locale). A differenza del widget pubblico, l'aggiunta manuale **non applica** il calcolo di disponibilità/capienza totale né il blocco overbooking, e il tavolo è opzionale (se non scelto, la prenotazione finisce nella coda "Senza tavolo"); nasce direttamente `CONFIRMED` (considerata già accettata da chi la inserisce), non `PENDING`. Il menu di scelta del tavolo, qui come in coda, esclude in tempo reale quelli occupati per la data/ora/durata scelte — non è possibile assegnarne uno già occupato nemmeno dall'aggiunta manuale. Un campo di ricerca permette di "pescare" i dati di un cliente già prenotato in precedenza (per nome, email o telefono) invece di reinserirli da zero.
...
- **Prenotazioni (§5.7)** — implementata in v1 con queste scelte di default (da confermare/rivedere con l'utente):
  ...
  - v1 **non combina automaticamente più tavoli** per un unico gruppo grande: se nessun tavolo singolo basta, la prenotazione resta `PENDING` senza tavolo assegnato (visibile nel tab "Senza tavolo" della coda) e l'admin decide a mano (anche accostando fisicamente due tavoli, o registrandola comunque senza tavolo dall'aggiunta manuale). Estensione v2 possibile se richiesta.
